...
def checkParams(args):
    ...
    if (args.cellsize != -1) and (args.numberofcells != -1):
        Exception("You should specify only one parameter: cellsize or numberofcells")
    if (args.numberofcells > 0) and (int(args.numberofcells) % 2 != 0):
        Exception("numbeofcells should be odd")
...
def slices(inputfname,nslices=1,percbottom=-1.0,perctop=-1.0,verbose=True): 
    ...
    if nslices == 1: # one slice from percbottom to perctop
        if (percbottom == -1.0) or (perctop == -1.0):
            raise Exception("You must especify nslices > 0 or bottom and and top > 0")        
